...
pthread_mutex_t creating_thread_mutex;
...
int main()
{
	...
	// Create the mutex for creating a new thread
	if (pthread_mutex_init(&creating_thread_mutex, NULL) != 0)
	{
		fprintf(stderr, "Error at line %d: Mutex init has failed.\n", __LINE__);
		exit(EXIT_FAILURE);
	}
	...
	while (session_cnt < MAX_THREADS)
	{
		...
		// Keep on trying to accept socket connections via new threads
	...
	int closed_thread_cnt = 0;
	while (closed_thread_cnt < MAX_THREADS)
	{
		pthread_join(tid[closed_thread_cnt], NULL);

		closed_thread_cnt += 1;
	}

	pthread_mutex_destroy(&file_io_mutex);
	pthread_mutex_destroy(&creating_thread_mutex);
...
void* MainProgramThread(void* arg)
{
	...
	// Only create one socket at a time
	// pthread_mutex_lock(&creating_thread_mutex);
	...
	// Socket is created...
	// pthread_mutex_unlock(&creating_thread_mutex);
	...
	else
	{
		...
		// Create the MainProgramLoop
		int client_file_descriptor = new_socket_fd;
